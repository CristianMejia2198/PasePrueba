            ...
            new APIService()
                ...
                .then((res) => {
                    let lista = res.data.data.map((item) => {
                        return { itemImageSrc: `https://imagenesredes.s3.us-east-2.amazonaws.com/${item.link_imagen}`, thumbnailImageSrc: `https://imagenesredes.s3.us-east-2.amazonaws.com/${item.link_imagen}`, alt: `imagen-no-${item.id_image}` };
                    });
